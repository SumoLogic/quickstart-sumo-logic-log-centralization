...
class AWSInspector(AWSResource):
    def __init__(self, props, *args, **kwargs):
        self.inspectorcli = boto3.client('inspector')
        self.RulePackage={
            "us-west-2": {
    ...
    def extract_params(self, event):
        ...
        return {
            "AssessmentTargetName": props.get("AssessmentTargetName"),
            "AssessmentTemplateName": props.get("AssessmentTemplateName"),
            "EnableAllEC2Instances": props.get("EnableAllEC2Instances"),
            "TopicArn": props.get("TopicArn"),
            "Tags": props.get("Tags"),
            "DurationInSeconds": props.get("DurationInSeconds"),
            "Region": props.get("Region"),
            "InstallAmazonInspectorApp": props.get("InstallAmazonInspectorApp"),
            "RemoveAWSResourcesOnDeleteStack": props.get("RemoveAWSResourcesOnDeleteStack")
        }
    def create(self, AssessmentTargetName, AssessmentTemplateName, EnableAllEC2Instances,TopicArn,
    Tags,DurationInSeconds,Region,InstallAmazonInspectorApp, *args, **kwargs):
        try:
            AssessmentTemplateArn = None
            if (InstallAmazonInspectorApp=='Yes'):
                print("Install Amazon Inspector App")
                list_template_response = self.inspectorcli.list_assessment_templates(
                    filter={
                ...
                if(len(list_template_response.get('assessmentTemplateArns')))>0:
                    ...
                    subscribe_response = self.inspectorcli.subscribe_to_event(
                    ...
                    list_target_response = self.inspectorcli.list_assessment_targets(
                        filter={
                    ...
                    if(len(list_target_response.get('assessmentTargetArns')))>0:
                        ...
                        if (EnableAllEC2Instances=='Yes'):
                            create_target_response = self.inspectorcli.create_assessment_target(
                            ...
                            Ec2Tags = []
                            for res in Tags.split(","):
                                tag = {'key': res.split('=')[0],'value': res.split('=')[1]}
                                Ec2Tags.append(tag)        

                            create_resource_group_response = self.inspectorcli.create_resource_group(
                    ...
                    RulePackages = []
                    RulePackages.append(self.RulePackage.get(Region).get("CVE"))
                    RulePackages.append(self.RulePackage.get(Region).get("CIS"))
                    RulePackages.append(self.RulePackage.get(Region).get("Security"))
                    RulePackages.append(self.RulePackage.get(Region).get("Network"))
                    RulePackages.append(self.RulePackage.get(Region).get("Runtime"))

                    create_template_response = self.inspectorcli.create_assessment_template(
            ...
            return {"AssessmentTemplateArn": AssessmentTemplateArn}, AssessmentTemplateArn
        except ClientError as e:
            print("Error in creating inspector %s" % e.response['Error'])
            raise
        except Exception as e:
            print("Error in creating inspector %s" % e)
            raise
        
    def update(self, AssessmentTargetName, AssessmentTemplateName, EnableAllEC2Instances, TopicArn,
    Tags,DurationInSeconds,Region,InstallAmazonInspectorApp, *args, **kwargs):
        self.create(self, AssessmentTargetName, AssessmentTemplateName, EnableAllEC2Instances,TopicArn,
    Tags,DurationInSeconds,Region,InstallAmazonInspectorApp, *args, **kwargs)
        print("updated teamplate and target for aws inspector")
        return {"UPDATE": "Successful"}, "OK"

    def delete(self, AssessmentTargetName, AssessmentTemplateName, TopicArn, RemoveAWSResourcesOnDeleteStack, *args, **kwargs):
        try:
            list_template_response = self.inspectorcli.list_assessment_templates(
                filter={
                    'namePattern': AssessmentTemplateName
            ...
            if (len(AssessmentTemplateArn)>0):         
                if (RemoveAWSResourcesOnDeleteStack == 'false'):
                    ...
                    Unsubscribe_response = self.inspectorcli.unsubscribe_from_event(
                        resourceArn=AssessmentTemplateArn,
                        event='ASSESSMENT_RUN_STARTED',
                        topicArn = TopicArn
                    )
                    ...
                    delete_template_response = self.inspectorcli.delete_assessment_template(
                        assessmentTemplateArn=AssessmentTemplateArn
                    ) 
                    if (len(AssessmentTargetArn)>0):
                        print("Delete Assessment Target")
                        delete_target_response = self.inspectorcli.delete_assessment_target(
                            assessmentTargetArn=AssessmentTargetArn
                        )
            else:
            ...
            print("Error in deleting inspector %s" % e.response['Error'])
            raise
        except Exception as e:
            print("Error in deleting inspector %s" % e)
            raise
